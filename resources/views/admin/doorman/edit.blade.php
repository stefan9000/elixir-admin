...
@section('content')
    <div class="app-container" id="root">
        <div class="container-heading">
            <a class="back" href="{{ route('admin_doorman_index') }}">Back to list</a>
        </div>
        <form @submit.prevent="sendData()">
            <h3>Editing @{{ user.first_name }} @{{ user.last_name }}</h3>
            ...
            <input type="text" v-model="user.date_of_birth" required/>
    ...
    <script type="text/javascript">
        const app = new Vue({
            ...
            data: {
                ...
                user: {
                    id: '{{ $doorman->id }}',
                    first_name: '{{ $doorman->first_name }}',
                    last_name: '{{ $doorman->last_name }}',
                    email: '{{ $doorman->email }}',
                    phone: '{{ $doorman->phone }}',
                    date_of_birth: '{{ $doorman->date_of_birth }}',
                    password: '',
                    password_confirmation: ''
                }
            },
            methods: {
                sendData() {
                    ...
                    this.$http.patch('/api/doormen/' + this.user.id, {
